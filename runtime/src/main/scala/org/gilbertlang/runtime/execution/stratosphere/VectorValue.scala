...
class VectorValue(var vector: Vector) extends Vector with Value with WrapAsScala {
  def write(out: DataOutput){
    out.writeUTF(vector.getClass().getName())
    out.writeInt(vector.size())
    
    writeVector(out)
  }
  
  def writeVector(out: DataOutput){
    if(vector.isInstanceOf[IOReadableWritable]){
      val iorw = vector.asInstanceOf[IOReadableWritable]
      iorw.write(out)
    }else{
      var counter = 0
  ...
  def read(in: DataInput){
    ...
    vector = ctor.newInstance(size: java.lang.Integer).asInstanceOf[Vector]
    
    readVector(in)
  }
  
  def readVector(in: DataInput){
    if(vector.isInstanceOf[IOReadableWritable]){
      val iorw = vector.asInstanceOf[IOReadableWritable]
      iorw.read(in)
    }else{
   ...
   */
  override def clone() = {
    new VectorValue(Helper.clone(vector))
  }
   ...
   */
  def divide(x: Double) = {
    new VectorValue(vector.divide(x))
  }
   ...
   * @return a Vector
   */
  def like() = new VectorValue(vector.like())
   ...
   * @throws CardinalityException if the cardinalities differ
   */
  def minus(x: Vector) = new VectorValue(vector.minus(x))
   ...
   * @return a new Vector
   */
  def normalize() = new VectorValue(vector.normalize())
   ...
   */
  def normalize(power: Double) = new VectorValue(vector.normalize(power))
   ...
   * @return a new Vector
   */
  def logNormalize() = new VectorValue(vector.logNormalize)
   ...
   * @return a new Vector
   */
  def logNormalize(power: Double) = new VectorValue(vector.logNormalize(power))
   ...
   * @return a new Vector
   */
  def plus(x: Double) = new VectorValue(vector.plus(x))
  /**
   ...
   * @throws CardinalityException if the cardinalities differ
   */
  def plus(x: Vector) = new VectorValue(vector.plus(x))
   ...
   * @return a new Vector
   */
  def times(x: Double) = new VectorValue(vector.times(x))
  /**
   ...
   * @throws CardinalityException if the cardinalities differ
   */
  def times(x: Vector) = new VectorValue(vector.times(x))
  /**
   ...
   */
  def viewPart(offset: Int, length: Int) = new VectorValue(vector.viewPart(offset, length))
  ...
  def cross(other: Vector) = {
    if(other.isInstanceOf[Subvector]){
      val subvector = other.asInstanceOf[Subvector]
      new MatrixValue(vector.cross(subvector))
    }else{
      throw new IllegalArgumentError("Cross of subvector needs another subvector as parameter")
    }
  }
...
object VectorValue{
  implicit def vector2VectorValue(vector: Vector) = new VectorValue(vector)
}